...
def loss_f(losses, buffer, opt, agent, target_network, batch_size, beta, gamma=1., device='cpu'):
    """ Compute td loss using torch operations only."""
    states, actions, rewards, next_states, is_done= buffer.sample(batch_size)

#    print(states)
    states = torch.tensor(states.astype('float32'), device=device).reshape(-1, 1)    # shape: [batch_size, *state_shape]
...
#    weights = torch.tensor(weights, device=device)
    next_states = torch.tensor(next_states.astype('float32'), device=device).reshape(-1, 1)
